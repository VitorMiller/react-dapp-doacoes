...
from dtos.ong_autenticada_dto import OngAutenticadaDto
...
async def checar_autorizacao(request: Request):
    ...
    area_da_ong = request.url.path.startswith("/perfilOng")
    if area_da_ong and not usuario:
...
def criar_token(id: int, nome: str, email: str) -> str:
    payload = {
        "id": id,
        "nome": nome,
        "email": email,
