...
async def post_cadastro_ong(cadastrar_ong_dto: CadastrarOngDTO):
    try:
        responsavel = Responsavel(id = 0, nome = cadastrar_ong_dto.nome_responsavel,
        ...
        ong = Ong(id = 0, nome = cadastrar_ong_dto.nome_ong,
            causa = cadastrar_ong_dto.causa_ong, email = cadastrar_ong_dto.email_ong,
            endereco = cadastrar_ong_dto.endereco_ong, cidade= cadastrar_ong_dto.cidade_ong,
            estado= cadastrar_ong_dto.estado_ong, carteira= cadastrar_ong_dto.carteira_ong,
            id_responsavel= responsavel_cadastrado.id
        )
        ong_cadastrada = OngRepo.inserir(ong)
        return {"success": True, "data": ong_cadastrada}
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Erro ao cadastrar responsável: {str(e)}")
...
async def post_cadastro_doacao(cadastrar_doacao_dto: CadastrarDoacaoDTO):
    try:
        ...
        doacao = Doacao(id = 0, nome_doador = cadastrar_doacao_dto.nome_doador,
